...
// 1. Преобразовать данные к виду чтобы у каждого пользователя отобразить только id и username
const one = arrayData.map(item => ({id:item.id, username:item.username}))

// 2. Получить из данных выше список всех эмейлов
const two = arrayData.reduce((acc, item) => [...acc, item.email], [])

// 3. Вывести пользователя с id === 9
const three = arrayData.find(item => item.id === 9)

// 4. Вывести всех пользователей из города 'Lebsackbury'
const four = arrayData.filter(item => item.address.city === 'Lebsackbury')

// 5. Вывести первых 5 пользователей
const five = arrayData.slice(0, 5)

// 6. Вывести всех пользователей в обратном порядке
const six = arrayData.reverse()
